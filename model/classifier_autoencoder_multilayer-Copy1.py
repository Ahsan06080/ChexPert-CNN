...
class Classifier(nn.Module):
    ...
    def forward(self, x, x_list):
        ...
        x0 = torch.cat((x0,x_list[0]),1)
        x0 = F.relu(self.conv64(x0))
        ## 64 X 64
        x1 = self.dense_block1(x0)
        x1 = self.trans_block1(x1)
        x1 = torch.cat((x1,x_list[1]),1)
        x1 = F.relu(self.conv128(x1))
        ###  32x32
        x2 = self.trans_block2(self.dense_block2(x1))
        x2 = torch.cat((x2,x_list[2]),1)
        # print(x2.shape)
        # print(x_list[2].shape)
        x2 = F.relu(self.conv256(x2))
        ### 16 X 16
        x3 = self.trans_block3(self.dense_block3(x2))
        x3 = torch.cat((x3,x_list[3]),1)
        x3 = F.relu(self.conv512(x3))
        ## 8 X 8
