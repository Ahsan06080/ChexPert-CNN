    ...
    def forward(self, x):
        ...
        for index, num_class in enumerate(self.cfg.num_classes):
            if self.cfg.attention_map != "None":
                feat_map = self.attention_map(feat_map)#2
                # print('X'*100)
                # print('feature map after attention  ')
                # print(feat_map.shape)
            ...
            if not (self.cfg.global_pool == 'AVG_MAX' or
                ...
                logit_map = classifier(feat_map)#3
                logit_maps.append(logit_map.squeeze())
                # print('X'*100)
                # print('logit map')
                # print(logit_map.shape)
            # (N, C, 1, 1)
            feat = self.global_pool(feat_map, logit_map)#4
            # print('X'*100)
            # print('feat after global pool')
            # print(feat.shape)    
            if self.cfg.fc_bn:
            ...
            logit = classifier(feat)#6
            # print('X'*100)
            # print('logit after classifier')
            # print(logit.shape) 
            # (N, num_class)
