...
from model.classifier_mod_AE_FB import Classifier,ConvAutoencoder  # noqa
...
def train_epoch(summary, summary_dev, cfg, args, model,conv_ae, dataloader,
                dataloader_dev, optimizer1,optimizer2, summary_writer, best_dict,
                dev_header):
    ...
    for step in range(steps):
        ...
        FE_img,x_b = conv_ae(image)
        image = image+FE_img
        target = target.to(device)
        output, logit_map,FM = model(image)

        # different number of tasks
        loss = 0
        loss_FE = F.binary_cross_entropy_with_logits(x_b, FM.detach())
        ...
        if summary['step'] % cfg.test_every == 0:
            ...
            summary_dev, predlist, true_list = test_epoch(
                summary_dev, cfg, args, model, conv_ae, dataloader_dev)
            time_spent = time.time() - time_now
...
def test_epoch(summary, cfg, args, model,conv_ae, dataloader):
    ...
    for step in range(steps):
        ...
        image = image.to(device)
        target = target.to(device)
        FE_img,_ = conv_ae(image)
        image = image + FE_img
        output, logit_map,_ = model(image)
        # different number of tasks
        for t in range(len(cfg.num_classes)):
...
def run(args):
    ...
    model = Classifier(cfg) ##Need to change
    if args.verbose is True:
    ...
    conv_ae =  DataParallel(conv_ae, device_ids=device_ids).to(device).train()
    if args.pre_train is not None:
        if os.path.exists(args.pre_train):
            ckpt = torch.load(args.pre_train, map_location=device)
            model.module.load_state_dict(ckpt)
    optimizer1 = get_optimizer(model.parameters(), cfg)
    optimizer2 = get_optimizer(conv_ae.parameters(), cfg)
    ...
    #                                       % src_folder)
    # print(os.path.join(args.save_path, 'train.csv'))
    # if rc != 0:
    ...
    for epoch in range(epoch_start, cfg.epoch):
        ...
        summary_train, best_dict = train_epoch(
            summary_train, summary_dev, cfg, args, model,conv_ae,
            dataloader_train, dataloader_dev, optimizer1,optimizer2,
        ...
        if save_best:
            torch.save(
                ...
                os.path.join(args.save_path,
                             'best{}_{.3f}.ckpt'.format(best_dict['best_idx'],mean_auc))
            )
            best_dict['best_idx'] += 1
            if best_dict['best_idx'] > cfg.save_top_k:
                best_dict['best_idx'] = 1
