...
#from model.classifier_AG import Classifier  # noqa
...
def train_epoch(summary, summary_dev, cfg, args,model_feedback,model, dataloader,
                dataloader_dev, optimizer_feedback,optimizer, summary_writer, best_dict,
                dev_header):
    ...
    for step in range(steps):
        ...
        output, feedback= model_feedback(image)
        #print(feedback)
        output, logit_map = model(image,feedback)
        ...
        if summary['step'] % cfg.test_every == 0:
            ...
            summary_dev, predlist, true_list = test_epoch(
                summary_dev, cfg, args, model_feedback,model, dataloader_dev)
            time_spent = time.time() - time_now
...
def test_epoch(summary, cfg, args,model_feedback, model, dataloader):
    ...
    for step in range(steps):
        ...
        image = image.to(device)
        target = target.to(device)
        output_0, feedback= model_feedback(image)
        model.upsampler = feedback
...
def run(args):
    ...
    model = EfficientNet.from_pretrained(cfg,False,[0,0,0,0],'efficientnet-b3', num_classes = 5) 
    if args.verbose is True:
    ...
    model_feedback = DataParallel(model_feedback, device_ids=device_ids).to(device).train()
    ...
    optimizer_feedback = get_optimizer(model_feedback.parameters(), cfg)
    ...
    for epoch in range(epoch_start, cfg.epoch):
        ...
        summary_train, best_dict = train_epoch(
            summary_train, summary_dev, cfg, args, model_feedback,model,
            dataloader_train, dataloader_dev, optimizer_feedback,optimizer,
