...
from model.classifier_mod import Classifier   # noqa
...
def cosine_similarityloss(logit_maps,labels,cfg) :
    cosine_similarity_list = []
    n_class = len(cfg.num_classes)
    ...
    cosine_similarity_tensor = torch.stack(cosine_similarity_list, dim =1)
    cosine_similarity_tensor = cosine_similarity_tensor.reshape(logit_maps[0].shape[0],n_class,n_class)
    n = cosine_similarity_tensor.shape[1]
    mask = torch.eye(n, n).bool().cuda()
    cosine_similarity_tensor = cosine_similarity_tensor[:].masked_fill_(mask, 0)
    return cosine_similarity_tensor.mean()
...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
    ...
    for step in range(steps):
        ...
        loss = 0
        loss_c = cosine_similarityloss(logit_map,target,cfg)
